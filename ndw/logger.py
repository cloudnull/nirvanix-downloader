...
class Logging(object):
    ...
    def logger_setup(self):
        ...
        if _log_level in avail_level:
            ...
            raise SystemExit('I died because no log level was set')

        if self.log_file:
            handler = log_handler.RotatingFileHandler(self.log_file,
                                                      maxBytes=150000000,
                                                      backupCount=5)
        else:
...
def return_logfile(filename, log_location):
    ...
    if os.path.isfile(filename):
        ...
        log_name = ('%s' % filename)
        logfile = os.path.join(log_location, log_name)
        return logfile
